...
export default function ConfiguracionPage() {
  ...
  const configuracionesGenerales = [
    ...
    { clave: 'logo_url', nombre: 'Subir logo (PNG/JPG/SVG)', tipo: 'file' },
    ...
    <AdminSection title={titulo}>
      <div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {clave === 'logo_url' ? (
                <>
                  ...
                  {editedConfig['logo_url'] && (
                      ...
                      <div className="w-20 sm:w-28 md:w-32 lg:w-40 aspect-square bg-slate-800 border border-slate-700/50 rounded-lg flex items-center justify-center overflow-hidden">
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img
                          src={editedConfig['logo_url']}
                          alt="Logo"
                          className="w-full h-full object-contain p-2 block"
                          onError={(e) => { (e.currentTarget as HTMLImageElement).src = '/icon-512x512.png' }}
                        />
                      </div>
          ...
          {/* Uploader duplicado eliminado: solo se mantiene el uploader asociado a la clave 'logo_url' dentro del mapeo de configuraciones */}
        </div>
