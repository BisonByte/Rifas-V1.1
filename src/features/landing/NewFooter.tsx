...
export function NewFooter() {
  ...
  const resolveIconSrc = (r: RedSocial): string | null => {
    ...
    // Map to uploaded brand icons (icons8-*), else fallback to inline SVG
    switch (p) {
      case 'instagram':
        return '/icons/instagram.svg'
      case 'tiktok':
        return '/icons/tik-tok.svg'
      case 'whatsapp':
  ...
  // Small helper to render an <img> with graceful fallback to inline SVG icon
  const IconImg = ({ src, alt, p }: { src: string; alt: string; p: SocialPlatform }) => {
    const [errored, setErrored] = useState(false)
    if (errored || !src) return <SocialIcon p={p} className="w-5 h-5" />
    return (
      <img
        src={src}
        alt={alt}
            ...
            <div className="flex justify-center md:justify-end gap-3">
              {(
                redes && redes.length > 0
                  ? redes
                  : [
                      { id: 'ig-fallback', nombre: 'Instagram', url: '#', icono: 'icons8-instagram.svg', activo: true },
                      { id: 'tt-fallback', nombre: 'TikTok', url: '#', icono: 'icons8-tik-tok.svg', activo: true },
                      { id: 'wa-fallback', nombre: 'WhatsApp', url: '#', icono: 'icons8-whatsapp.svg', activo: true }
                    ]
              ).map((red) => {
                ...
                return (
                  ...
                  >
                    {src ? (
                      <IconImg src={src} alt={red.nombre} p={p} />
                    ) : (
