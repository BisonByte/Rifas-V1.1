...
export async function POST(
  ...
  try {
    ...
    // Si no tiene premios, crear uno por defecto automáticamente
    if (!rifa.premios || rifa.premios.length === 0) {
      await prisma.premio.create({
        data: {
          rifaId: rifa.id,
          titulo: `Premio principal – ${rifa.nombre}`,
          descripcion: rifa.descripcion?.slice(0, 200) || 'Premio principal',
          cantidad: 1,
          orden: 1,
        }
      })
  ...
  return NextResponse.json({ success: true, data: publicada, message: 'Rifa publicada (activa). Se creó un premio por defecto si no existía.' })
  } catch (error) {
